...
func (s *Store) RenewLease(ctx context.Context, kind LeaseKind, id int64, tok LeaseToken, until time.Time) error {
	var table, statusGuard string
	var statuses []any
	switch kind {
	case LeaseRepo:
		table = "repos"
		statusGuard = "status = ?"
		statuses = []any{string(RepoListing)}
	case LeaseFile:
		table = "files"
		statusGuard = "status IN (?, ?, ?)"
		statuses = []any{string(FileDownloading), string(FileSalvaging), string(FileVerifying)}
	case LeaseBlock:
		table = "blocks"
		statusGuard = "status = ?"
		statuses = []any{string(BlockActive)}
	case LeaseReference:
		table = "reference_files"
		statusGuard = "status = ?"
		statuses = []any{string(RefHashing)}
	default:
		return fmt.Errorf("store: renew lease: unknown lease kind %q", string(kind))
	}
	now := utc(time.Now())
	query := fmt.Sprintf(
		"UPDATE %s SET lease_until = ?, updated_at = ? WHERE id = ? AND lease_token = ? AND lease_until IS NOT NULL AND lease_until > ? AND %s",
		table, statusGuard)
	args := []any{utc(until), now, id, string(tok), now}
	args = append(args, statuses...)
	return execGuarded(ctx, s.db, "renew lease", id, query, args...)
}
